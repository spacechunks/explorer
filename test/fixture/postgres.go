...
func (p *Postgres) CreateFlavor(t *testing.T, chunkID string, f *chunk.Flavor) {
	...
	// now we need to update the flavor id, because
	// it is generated by the CreateFlavor call and
	// what is currently in place is not a valid id.
	for i, _ := range tmp {
		tmp[i].FlavorID = createdFlavor.ID
	}

	createdFlavor.Versions = tmp

	for i, _ := range createdFlavor.Versions {
		p.CreateFlavorVersion(t, &createdFlavor.Versions[i])
	}
...
func (p *Postgres) CreateFlavorVersion(t *testing.T, version *chunk.FlavorVersion) {
	ctx := context.Background()
	created, err := p.DB.CreateFlavorVersion(ctx, *version, "")
	require.NoError(t, err)
	*version = created
}
