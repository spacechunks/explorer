...
func TestChunkOwner(t *testing.T) {
	...
	actual, err := pg.DB.ChunkOwner(ctx, c.ID, c.Owner.IDPID)
...
func TestFlavorOwner(t *testing.T) {
	...
	actual, err := pg.DB.FlavorOwner(ctx, c.Flavors[0].ID, c.Owner.IDPID)
	require.NoError(t, err)
...
func TestFlavorVersionOwner(t *testing.T) {
	...
	actual, err := pg.DB.FlavorVersionOwner(ctx, c.Flavors[0].Versions[0].ID, c.Owner.IDPID)
	require.NoError(t, err)
