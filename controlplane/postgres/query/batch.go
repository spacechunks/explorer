...
const insertBlobs = `-- name: InsertBlobs :batchexec
/*
 * BLOB STORE
 */
...
type InsertBlobsBatchResults struct {
	br     pgx.BatchResults
	tot    int
	closed bool
}

type InsertBlobsParams struct {
	Hash      string
	SizeBytes int64
}

func (q *Queries) InsertBlobs(ctx context.Context, arg []InsertBlobsParams) *InsertBlobsBatchResults {
	batch := &pgx.Batch{}
	for _, a := range arg {
		vals := []interface{}{
			a.Hash,
		...
		batch.Queue(insertBlobs, vals...)
	}
	br := q.db.SendBatch(ctx, batch)
	return &InsertBlobsBatchResults{br, len(arg), false}
}

func (b *InsertBlobsBatchResults) Exec(f func(int, error)) {
	defer b.br.Close()
	for t := 0; t < b.tot; t++ {
		if b.closed {
			if f != nil {
...
func (b *InsertBlobsBatchResults) Close() error {
	b.closed = true
	return b.br.Close()
}
