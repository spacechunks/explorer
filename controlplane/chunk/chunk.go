...
func (s *svc) CreateChunk(ctx context.Context, chunk resource.Chunk) (resource.Chunk, error) {
	...
	if !ok {
		return resource.Chunk{}, errors.New("actor_idp_id not found in context")
	}

	u, err := s.userRepo.GetUserByIDPID(ctx, actorID)
	if err != nil {
...
func (s *svc) ListChunks(ctx context.Context, pageSize int, afterID *string) ([]resource.Chunk, error) {
	return s.repo.ListChunks(ctx, pageSize, afterID)
}
